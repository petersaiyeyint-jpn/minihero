(() => {
  ...
  const refreshApp = () => {
    writeState(ensureArrays(readState()));
  ...
  const renderBudgets = () => {
    ...
    host.innerHTML = rows.length ? rows.map((row) => `
      <div class="budget-category-management-row">
        <div class="budget-category-management-meta"><strong>${escapeHtml(row.category)}</strong><small>${escapeHtml(row.month)} · ${amount(row.amount).toLocaleString('en-US')} MMK</small></div>
        <div class="budget-category-management-actions"><button type="button" data-bcm-edit-budget="${escapeHtml(row.id)}">Edit</button><button type="button" class="delete" data-bcm-delete-budget="${escapeHtml(row.id)}">Delete</button></div>
      </div>
  ...
  const renderCategories = () => {
    ...
    host.innerHTML = state.categories.length ? state.categories.map((category) => `
      <div class="budget-category-management-row">
        <div class="budget-category-management-meta"><strong>${escapeHtml(category.name)}</strong><small>${escapeHtml(category.type || 'expense')}</small></div>
        <div class="budget-category-management-actions"><button type="button" data-bcm-edit-category="${escapeHtml(category.id)}">Edit</button><button type="button" class="delete" data-bcm-delete-category="${escapeHtml(category.id)}">Delete</button></div>
      </div>
  ...
  const submitBudget = (form) => {
    ...
    else state.budgets.push({ id: id('budget'), category, month: selectedMonth, amount: value, createdAt: new Date().toISOString() });
  ...
  const submitCategory = (form) => {
    ...
    if (state.categories.some((item) => String(item.name).toLowerCase() === name.toLowerCase())) return notify('This category already exists.', true);

    state.categories.push({ id: id('category'), name, type, createdAt: new Date().toISOString() });
    writeState(state);
    form.reset();
    refreshApp();
    render();
  ...
  const handleClick = (event) => {
    ...
    if (editBudget) {
      ...
      writeState(state); refreshApp(); render(); notify('Budget updated.');
      return;
    }
    ...
    if (deleteBudget) {
      ...
      writeState(state); refreshApp(); render(); notify('Budget deleted.');
      return;
    }
    ...
    if (editCategory) {
      ...
      writeState(state); refreshApp(); render(); notify('Category updated.');
      return;
    }
    ...
    if (deleteCategory) {
      ...
      writeState(state); refreshApp(); render(); notify('Category deleted.');
    }
  };
